...
import DataTable from "../../components/DataTable";
import FormModal from "../../components/FormModal";
import { getTeachers, createTeacher, updateTeacher, deleteTeacher, getDepartments,getClasses } from "../../services/api";
...
const AdminTeachers = () => {
  ...
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState(null);
  const [form, setForm] = useState({ name: "", email: "", password: "", department: "",classId:"" });

  const fetchData = async () => {
    try {
      const [tchs, depts,clss] = await Promise.all([getTeachers(), getDepartments(),getClasses()]);
      setData(tchs.data.data);
      setDepartments(depts.data.data);
      setCls(classId.data.data);
    } catch (err) { console.log(err); }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setModalOpen(false); setEditing(null); setForm({ name: "", email: "", password: "", department: "" ,classId:""}); fetchData();
    } catch (err) { console.log(err); }
  };

  const handleEdit = (row) => {
    setEditing(row);
    setForm({ name: row.name, email: row.email, password: "", department: row.department?._id || row.department || "" , classId: row.classId?._id || row.classId || "" });
    setModalOpen(true);
  };
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div className="page-header mb-0"><h1>Teachers</h1><p>Manage college teachers</p></div>
        <button onClick={() => { setEditing(null); setForm({ name: "", email: "", password: "", department: "",classId:"" }); setModalOpen(true); }}
          className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-primary text-primary-foreground font-medium hover:opacity-90 transition-opacity">
          <Plus className="w-4 h-4" /> Add Teacher
        </button>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select value={form.classId} onChange={(e) => setForm({ ...form, classId: e.target.value })}
              ...
              {departments.map((classId) => <option key={classId._id} value={classId._id}>{classId.name}</option>)}
            </select>
          </div>
          <button type="submit" className="w-full py-2.5 rounded-lg bg-primary text-primary-foreground font-medium hover:opacity-90 transition-opacity">
            {editing ? "Update" : "Create"}
